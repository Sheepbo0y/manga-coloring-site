...
#### 用户免费次数系统
- [x] 每个新注册用户默认获得 5 次免费上色次数
- [x] 用户每次调用上色 API 时扣除 1 次次数
- [x] 次数用完时提示用户（明天配置 Seedream API Key 后可充值）
- [x] 在用户界面显示剩余次数（Header 用户菜单、上传页面）

#### 管理员后台
- [x] 创建 /admin 路由和管理员页面
- [x] 管理员可以查看所有用户列表
- [x] 管理员可以为指定用户增加/减少免费次数
- [x] 管理员可以查看系统统计数据（总用户数、总作品数、总调用次数等）
- [x] 只有特定管理员账号可以访问（通过用户 ID 或邮箱判断）

#### 数据库变更
- [x] User 模型添加 freeCredits 字段（默认 5）
- [x] User 模型添加 isAdmin 字段（默认 false）
- [x] 添加 CreditLog 记录次数变动历史
- [x] 执行 Prisma 迁移

#### API 端点
- [x] GET /api/users/me/credits - 获取当前用户剩余次数
- [x] GET /api/admin/stats - 获取系统统计（管理员）
- [x] GET /api/admin/users - 获取所有用户列表（管理员）
- [x] POST /api/admin/users/:id/credits - 调整用户次数（管理员）
- [x] GET /api/admin/users/:id/credit-logs - 获取次数变动历史（管理员）

#### 前端页面和组件
- [x] Header 组件 - 显示剩余次数
- [x] UploadPage - 显示剩余次数和次数不足提示
- [x] AdminDashboard - 管理员后台首页（统计 + 用户管理）

#### 类型定义
- [x] User 类型添加 isAdmin, freeCredits 字段
- [x] 新增 CreditLog 类型
- [x] 新增 AdminStats 类型
...
8. ✅ **性能优化** - 图片压缩、代码分割
9. ✅ **高级功能** - 搜索筛选、通知系统
10. ✅ **用户次数系统** - 免费次数、扣除逻辑、管理员后台

### 🟡 待完成功能（可选）
11. **社交分享** - 分享到微信、微博等
12. **数据统计** - 访问统计、用户行为分析
13. **PWA 支持** - 离线访问、添加到主屏幕
